...
class krxModule:
    ...
    def chartJSON(self, dic):
        ...
        rand = [[np.random.randint(0, 255), np.random.randint(
            0, 255), np.random.randint(0, 255)] for _ in range(len(labels))]

        # bg생상과 테두리 색상 만들어줌
        backgroundColors = ['rgba(%d,%d,%d,0.2)' % (
            rand[i][0], rand[i][1], rand[i][2]) for i in range(len(labels))]
        borderColors = ['rgba(%d,%d,%d,1)' % (
            rand[i][0], rand[i][1], rand[i][2]) for i in range(len(labels))]
